...
VERSION="${STTDB_VERSION:-latest}"
if [ "$VERSION" = "latest" ]; then
  VERSION=$(curl -fsSL "https://api.github.com/repos/${REPO}/releases/latest" | grep '"tag_name"' | cut -d'"' -f4)
  [ -n "$VERSION" ] || error "Failed to fetch latest version."
fi
info "Installing sttdb $VERSION"

# ── Download ────────────────────────────────────────────

ARCHIVE="sttdb-${VERSION}-${PLATFORM}.tar.gz"
...
cp "$TMPDIR/sttdb" "$INSTALL_DIR/sttdb"
...
info "Done! Run 'sttdb --help' to get started."
