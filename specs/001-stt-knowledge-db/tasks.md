...
- [ ] T001 Initialize Go module (`go mod init`) and create project directory structure per plan.md: `cmd/tacit/`, `pkg/audio/`, `pkg/stt/`, `pkg/process/`, `pkg/storage/`, `pkg/config/`, `pkg/daemon/`, `pkg/pipeline/`, `testdata/`
...
**Independent Test**: Run `tacit start`, speak into mic, verify markdown file created in `~/.tacit/{category}/` with YAML frontmatter (title, category, created_at) and body (summary + STT transcript)
...
- [ ] T005 [P] [US1] Write STT integration test in `pkg/stt/whisper_integration_test.go`: `TestWhisperTranscribe_RealAudio` — decode `testdata/test_voice_recording.m4a` → transcribe → assert non-empty text. Skip if `TACIT_WHISPER_MODEL` env not set. See plan.md Integration Test: STT section.
...
- [ ] T010 [P] [US1] Implement Whisper model loading in `pkg/stt/model.go`: Model path resolution from config (`~/.tacit/models/ggml-{model}.bin`), existence check, and loading via whisper.cpp Go bindings.
...
- [ ] T013 [P] [US1] Implement PID file management with stale detection in `pkg/daemon/pid.go`: `WritePID(path string)`, `ReadPID(path string) (int, error)`, `IsRunning(pid int) bool` (signal 0 check), `RemovePID(path string)`, stale PID auto-cleanup. PID file at `~/.tacit/tacit.pid`. Include unit tests.
...
- [ ] T019 [US1] Implement CLI entry point with start/stop/status daemon commands in `cmd/tacit/main.go`: `tacit start` (run pipeline in foreground, write PID, handle SIGINT/SIGTERM for graceful shutdown), `tacit stop` (read PID, send SIGTERM, remove PID file), `tacit status` (check PID liveness, report running/stopped). Use `pkg/daemon` for PID management, `pkg/pipeline` for pipeline, `pkg/config` for configuration.

**Checkpoint**: User Story 1 complete. Run `tacit start` → speak → verify markdown file in `~/.tacit/`. Run `tacit stop` → verify daemon terminates. Integration tests pass with `go test -tags integration ./...`
...
**Independent Test**: Pre-populate `~/.tacit/` with sample markdown files, register MCP server with Claude Code, ask Claude a question about stored knowledge, verify it references knowledge DB content
...
- [ ] T024 [US2] Implement MCP server with search_knowledge, list_knowledge, get_knowledge tools in `cmd/tacit/main.go` (as `mcp` subcommand): Use `modelcontextprotocol/go-sdk` with stdio transport. Register 3 tools per contracts/mcp-tools.md schemas. `search_knowledge` → `storage.Search()`, `list_knowledge` → `storage.List()`, `get_knowledge` → `storage.Read()`. Validate `get_knowledge` ID against path traversal. Return `isError: true` for missing entries. Knowledge base root: `~/.tacit/`.

**Checkpoint**: MCP server works. Register with `claude mcp add --transport stdio tacit --scope user -- tacit mcp`. Ask Claude about stored knowledge. All 3 tools functional.
...
**Independent Test**: With knowledge entries stored, run `tacit search "에러"` and `tacit list --category 개발` to verify results display correctly

### Tests for User Story 3

> **NOTE: Write these tests FIRST, ensure they FAIL before implementation.**

- [ ] T025 [P] [US3] Write CLI search command unit tests in `cmd/tacit/main_test.go`: Test argument parsing (`tacit search <query> --category <cat> --include-chitchat`), output formatting (title, category, date, summary per result), error on missing query argument. Use `storage.Search()` with a temp directory fixture.
- [ ] T026 [P] [US3] Write CLI list command unit tests in `cmd/tacit/main_test.go`: Test argument parsing (`tacit list --category <cat> --date-range <range>`), tabular output format, empty results message. Use `storage.List()` with a temp directory fixture.

### Implementation for User Story 3

- [ ] T027 [US3] Add search command to CLI in `cmd/tacit/main.go`: `tacit search <query> [--category <cat>] [--include-chitchat]` — call `storage.Search()`, display results with title, category, date, and summary. Format output for terminal readability.
- [ ] T028 [US3] Add list command with category filter to CLI in `cmd/tacit/main.go`: `tacit list [--category <cat>] [--date-range <start>..<end>]` — call `storage.List()`, display tabular list of entries with title, category, date.
...
- [ ] T030 Run quickstart.md validation: verify complete setup from scratch (go mod download, go build, go test, whisper model download, `tacit start/stop/status`, MCP server registration)
...
- T027-T028: sequential (both modify cmd/tacit/main.go)
...
Task T019: "Implement CLI in cmd/tacit/main.go"
```
...
Task T024: "Implement MCP server as 'mcp' subcommand in cmd/tacit/main.go"
```
...
4. **STOP and VALIDATE**: `tacit start` → speak → verify markdown file created → `tacit stop`
5. Run integration tests: `TACIT_WHISPER_MODEL=~/.tacit/models/ggml-base.bin go test -tags integration -v ./...`
...
- Integration tests require: Whisper model file (`TACIT_WHISPER_MODEL` env) + Claude Code CLI (`claude`)
